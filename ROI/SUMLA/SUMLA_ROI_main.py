...
def first_filter(img):
    ...
    # 高斯滤波
    # img_GaussianBlur = cv2.GaussianBlur(img, (3, 3), 5)
    # 高斯双边滤波

    img_bilateralFilter = cv2.bilateralFilter(img, 19, 0.1, 210)
    # img_bilateralFilter = contrast(img_bilateralFilter, 0, 10)
    # img_bilateralFilter = cv2.bilateralFilter(img, 41, 0.1, 255)
...
def edge_detection(img):
    ...
    # img2 = contrast(img, 0, 20)
    # gray_lap = cv2.Laplacian(img, cv2.CV_16S, ksize=3)
    # img_edge = cv2.convertScaleAbs(gray_lap)  # 轉回uint8
    ...
    img_edge = cv2.add(absX, absY)
    # 與原圖相同亮度 所以在做相減時 與原圖相同元素砍掉 只留下邊邊亮的地方 且因為邊邊比原本亮 所以與原圖相減後是呈現最暗
    img_edge = cv2.addWeighted(img, 0.2, img_edge, 0.8, 0)
    # img_edge = cv2.add(img, img_edge)
    # img_edge = cv2.add(img, img_edge)
    # img_edge = contrast(img_edge, 0, 10)
    # img = contrast(img, 0, 10)
    # img_edge = cv2.subtract(img, img_edge)
    # img_edge = contrast1(img_edge, 0, 1.4)
    # img_edge = contrast(img_edge, 0, 20)
    # img_edge = contrast1(img_edge, 2, 1.4)
    # img_edge = cv2.adaptiveThreshold(img_edge,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,11,2)
    # ret3,img_edge = cv2.threshold(img_edge,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # ret,img_edge = cv2.threshold(img_edge,1,255,cv2.THRESH_BINARY)
    # ret,img_edge = cv2.threshold(img_edge,127,255,cv2.THRESH_TOZERO)

    # gray_lap = cv2.Laplacian(img_edge, cv2.CV_16S, ksize=3)
    # img_edge = cv2.convertScaleAbs(gray_lap)  # 轉回uint8
    # img_edge = cv2.add(img, img_edge)

    canny_th3 = cv2.Canny(img_edge, 130, 190)
    ...
    final_edge = cv2.erode(MORPH_CLOSE, kernel, iterations=3)
...
def contrast1(img, brightness=0, contrast=3):
    ...
    array_alpha = np.array([float(contrast)])  # contrast
    array_beta = np.array([float(brightness)])  # brightness
...
def contrast(img, brightness=0, contrast=100):
...
def positioning_middle_point(img, dst, point_pixel):
    ...
    while (not low_r or not high_r) and w2 > (w // 2):
        ...
        if (not low_r or not high_r):
            w2 = w2 - 2
        elif (abs(higher_right-lower_right) < 85):
            w2 = w2 - 2
            low_r = False
            high_r = False
        # elif (abs(higher_right-lower_right) < 95):
        #     w2 = w2 - 2
        #     low_r = False
        #     high_r = False
        # elif (abs(higher_right-lower_right) < 90):
        #     w2 = w2 - 2
        #     low_r = False
        #     high_r = False
        # elif (abs(higher_right-lower_right) < 85):
...
def get_imgs_roi(img_file, save_path, expcetion_path, roi_result_path):
    ...
    # print(all_folder)
    # print(len(all_folder))
    # os.makedirs("./expcetion_dst")
    if not os.path.isdir("./expcetion_dst"):
        os.makedirs("./expcetion_dst")
    for _, a_folder in enumerate(all_folder):
        # print(a_folder)
        child_folder = os.listdir(img_file+a_folder)
        # print(child_folder)
        for _, images in enumerate(child_folder):
            if not os.path.isdir(roi_result_path+"draw_"+a_folder+"/"+images):
                os.makedirs(roi_result_path+"draw_"+a_folder+"/"+images)
            for i, image in enumerate(os.listdir(img_file+a_folder+"/"+images)):
                img_raw = cv2.imread(os.path.join(
                    img_file+a_folder+"/"+images+'/', image), 0)
                print(img_file+a_folder+"_"+images+'_'+image)
                ...
                try:
                    ...
                    # roi图像保存路径
                    # new_file = save_path + 'ROI_'+a_folder+"/"+images
                    new_file = save_path+a_folder+"_"+images+"_"+image
                    # print(new_file)
                    # save_root = os.path.join(new_file, image)

                    new_file2 = roi_result_path + 'draw_'+a_folder+"/"+images
                    ...
                    img_raw = cv2.imread(os.path.join(
                        img_file+a_folder+"/"+images+'/', image), 0)
                    _file = expcetion_path+a_folder+"-"+images+"-"+image
                    # save_root = os.path.join(new_file, image)
                    cv2.imwrite(_file, img_raw)
                    # _file2 = expcetion_path+"dst/"+"dst_"+images+"-"+image
                    _file2 = "./expcetion_dst/"+"dst_"+a_folder+"-"+images+"-"+image
                    # save_root = os.path.join(new_file, image)
