...
def perf_measure2(model, x_test, y_test):
    ...
    # thred = uniques[y]
    thred = np.arange(Min, Max+step_len, step_len)
    ...
    for threshold in thred:
        ...
        for i in range(123):
            ...
            # TP_sum = TP.sum()
            # TN_sum = TN.sum()
        # print(threshold ,FAR_FRR(fp,fn, tp, tn))
        FAR, FRR, TPR = FAR_FRR(fp,fn, tp, tn)
        far.append(FAR)
        frr.append(FRR)
        tpr.append(TPR)
    ...
    eer_threshold = interp1d(far, thred)(EER)
    print("EER", EER)
    ...
    plt.figure()
    plt.plot(far, tpr, label='ROC curve (AUC = %0.2f)' % roc_auc, marker = '',lw=2)
    ...
    plt.figure()
    plt.plot(frr, far, color = 'green', marker = '',label = 'ROC',lw=2)
    plt.legend()
    plt.xlim([-0.005, 1.005])
    plt.ylim([-0.005, 1.005])
    plt.xlabel('FRR')
